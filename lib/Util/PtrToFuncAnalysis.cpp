#include "Util/PtrToFuncAnalysis.h"
...
bool PtrToFuncAnalysis::analyze(std::string globalVariableName) {
...
void PtrToFuncAnalysis::collectFunction(const llvm::Function *F) {
	if(!F) return;
...
void PtrToFuncAnalysis::collectFunction(llvm::CallSite CS) {
	const llvm::Function *F = CS.getCalledFunction();
	collectFunction(F);
}

void PtrToFuncAnalysis::collectVariables(const DPItem &item, ConstraintEdge *edge) {
...
void PtrToFuncAnalysis::collectBackwardNodes(ConstraintNode *curNode, ConstraintNode *prevNode) {
	auto &inEdges = curNode->getInEdges();
...
inline void PtrToFuncAnalysis::forwardProcess(const DPItem &item) {
...
void PtrToFuncAnalysis::forwardpropagate(const DPItem &item, ConstraintEdge *edge) {
...
inline void PtrToFuncAnalysis::backwardProcess(const DPItem &item) {
...
void PtrToFuncAnalysis::backwardpropagate(const DPItem &item, ConstraintEdge *edge) {
	const ConstraintNode* curNode = getNode(item.getCurNodeID());
